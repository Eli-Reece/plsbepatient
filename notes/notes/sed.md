...
`sed -i 's/colour/color/g'`: The sed command to run (in-place edit, global
substitution)

`{} +`: `{}` is replaced with the filename, and `+` tells find to pass as many
filenames as possible to one invocation of sed
...
```bash
$ find . -name "*.c" -exec sed -i.bak 's/foo/bar/g' {} +
```
...
Add a new line that has "New line here" after each line containing "START" in .conf files:
```bash
